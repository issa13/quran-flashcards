...
const INDEX_SURAHS_FILE = path.join(__dirname, "quran-index", "surahs.json");
const TOTAL_PAGES = 604;
const EXPECTED_SURAHS = 114;
const WRITE = process.argv.includes("--write");
const LOOKAHEAD = 5; // how many surahs ahead to search for a plausible match per header

const SURAH_NAMES = JSON.parse(fs.readFileSync(INDEX_SURAHS_FILE, "utf8"))
...
function main() {
  ...
  for (const h of headers) {
    ...
    if (originalSurahNum !== matched || h.lineObj.text !== correctName) {
