...
// closely as possible — same distractor strategies, just ported to
// Deno/TypeScript. Text now comes from the SAME locally-derived index
// (quran-index/ayahs.json + surahs.json — see derive-quran-index.js)
// that app.js and quran-reader.js already use client-side, fetched
// here via an absolute URL since a server function has no document
// origin of its own to resolve a relative path against. This was the
// one remaining place in the app still depending on alquran.cloud —
// duels now run fully offline of it too, same as everything else.
//
...
// If you've forked this to your own domain, set QURAN_DATA_BASE_URL
// (a Supabase function secret) to your deployment's base URL —
// `supabase secrets set QURAN_DATA_BASE_URL=https://you.github.io/your-repo/`
// — otherwise it defaults to this project's own GitHub Pages site.

import { createClient } from "https://esm.sh/@supabase/supabase-js@2";

const DATA_BASE_URL = Deno.env.get("QURAN_DATA_BASE_URL") || "https://issa13.github.io/quran-flashcards/";
...
// `number` is now a "surah:ayah" reference string (e.g. "2:255"),
// not alquran.cloud's global 1-6236 integer — matches what
// fetchAyahAudioUrl() on the client already accepts for solo/offline
// mode (see app.js), so nothing on the client needed to change for
// duels to switch over to it too.
interface Ayah {
  number: string;
...
interface GeneratedQuestion {
  ...
  page: number | null;
  q_text: string;
  q_ayah_number: string | null;
  is_audio_only: boolean;
  choices: string[];
  correct_index: number;
}

// Raw shapes of the two locally-derived index files, as produced by
// derive-quran-index.js — same files app.js loads client-side.
...
// -------- local data, bulk-loaded once per warm isolate --------
// Both index files are small (a few hundred KB total) and static, so
// pulling each down ONCE and slicing it in memory is both simpler and
// far fewer round-trips than the old per-page alquran.cloud calls
// (which fired off a fresh request for every single page touched
// while generating a duel's whole question set). The module-level
// promises persist across invocations for as long as the function
// instance stays warm, same as `pageCache` below already did.
let ayahIndexPromise: Promise<RawAyahEntry[]> | null = null;
function fetchLocalAyahIndex(): Promise<RawAyahEntry[]> {
  if (ayahIndexPromise) return ayahIndexPromise;
  ayahIndexPromise = fetch(`${DATA_BASE_URL}quran-index/ayahs.json`)
    .then((res) => { if (!res.ok) throw new Error("HTTP error fetching ayahs.json"); return res.json(); })
    .catch((e) => { ayahIndexPromise = null; throw e; });
  return ayahIndexPromise;
}

let surahIndexPromise: Promise<RawSurahEntry[]> | null = null;
function fetchLocalSurahIndex(): Promise<RawSurahEntry[]> {
  if (surahIndexPromise) return surahIndexPromise;
  surahIndexPromise = fetch(`${DATA_BASE_URL}quran-index/surahs.json`)
    .then((res) => { if (!res.ok) throw new Error("HTTP error fetching surahs.json"); return res.json(); })
    .catch((e) => { surahIndexPromise = null; throw e; });
